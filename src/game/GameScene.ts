class GameScene extends egret.DisplayObjectContainer{
	...
	private initView(){
		...
		// GameScene本身就是egret.DisplayObjectContainer
	...
	private update(timeStap:number):boolean{
		...
		for( let obj of this.gameObjectList){
			obj.update(timeStap); // 多态的移动方法
		}
		//添加元素
		this.addElement(); // 障碍物生成
		this.collision(); //膨胀检测
		return true;
	...
	private addElement(){
		...
		if( element && GameData.distance >= element.distance + GameData.rounds * GameData.maxMileage){
			...
			if( GameData.elements.length <= 0){// 这儿做的是个长度检测
	...
	private collision(){
		...
		for( let barrier of this.gameObjectList){
			...
			for( let obj of this.deleteObjectList){ // 统一移除对象
